...
class Movie(AbstractBaseModel):
    ...
    def get_similar_movies(self):
        ...
        return (
            Movie.objects
            .filter(genre__in=self.genre.all())
            .exclude(id=self.id)
            .distinct()[:5]
        )
...
class MovieLikes(AbstractBaseModel):
    ...
    movie = models.ForeignKey("kinogo.Movie", models.CASCADE, related_name="movie_likes")
...
class MovieDislikes(AbstractBaseModel):
    user = models.ForeignKey("users.User", models.CASCADE, related_name="movie_dislikes")
    movie = models.ForeignKey("kinogo.Movie", models.CASCADE, related_name="movie_dislikes")
...
class MovieComment(AbstractBaseModel):
    user = models.ForeignKey("users.User", on_delete=models.CASCADE, related_name="movie_comments")
    movie = models.ForeignKey("kinogo.Movie", on_delete=models.CASCADE, related_name="movie_comments")
